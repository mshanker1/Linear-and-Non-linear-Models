...
```{r}
fit3 <- principal(olymp[,-11],nfactors = 2,scores=T)
fit3
```
...
```{r}
fit.km$centers
aggregate(putil[-9],by=list(cluster=clusters),mean) #scaled form
```
...
set.seed(1234)
attach(fm.train)
ctree <- rpart(return~GPA+Miles.from.Home+College+Accommodations+Part.Time.Work.Hours+Attends.Office.Hours+High.School.GPA,data=fm.train,method="class",
  parms=list(split="information"))
summary(ctree)
ctree$cptable
plotcp(ctree)
